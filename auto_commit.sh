...
function logger() {
  ...
  case "$1" in
    debug)
      echo -e "$log_time \033[36mDEBUG\033[0m $2"
      ;;
    info)
      echo -e "$log_time \033[32mINFO\033[0m $2"
      ;;
    warn)
      echo -e "$log_time \033[33mWARN\033[0m $2"
      ;;
    error)
      echo -e "$log_time \033[31mERROR\033[0m $2"
      ;;
    *)
      ;;
  esac
...
if ! git pull --rebase origin main; then
    logger error "拉取失败，正在恢复本地更改..."
    git stash pop
    logger error "拉取失败，本地更改已恢复，请手动解决冲突后重新运行脚本。"
    exit 1
fi
